...
void main() {
   ...
   do {
      ...
      enable_interrupts(int_ext);      // Interrupci�n externa RB0
      Ext_int_edge(H_to_L);
      enable_interrupts(int_TIMER0);   // Interrupci�n de timer0
      enable_interrupts(int_TIMER1);   // Interrupcion de timer1
      enable_interrupts(GLOBAL);       // Llave maestra

      // Seteo de timer1 seg�n la frecuencia de an�lisis
      Seteo_Senal();
...
void Seteo_Senal() {
   // Se calcula N_tmr1 mediante la ecuaci�n T = 4/fosc * 256 * (256 - N_tmr1), prescaler = 256
